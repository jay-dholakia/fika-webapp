/**
 * IANA timezone per market slug for Fika slot wall-clock times (scalable: one zone per market).
 * Unknown slugs fall back to America/Los_Angeles.
 */

export const DEFAULT_MARKET_IANA_TIMEZONE = 'America/Los_Angeles'

/** Primary metro / dominant TZ for each slug in `lib/markets.ts` MARKETS. */
...
/** When both users share a market, use it; otherwise first non-empty (same match should usually agree). */
