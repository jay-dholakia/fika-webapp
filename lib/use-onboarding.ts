  ...
  useEffect(() => {
    ...
    async function load() {
      const [profileRes, intakeRes] = await Promise.all([
        supabase!.from('profiles').select('id, first_name, birthdate, gender, gender_preference, age_preference, languages, city, lat, lng, market, phone, avatar_url, id_verified_at, intent_confirmed_at, in_match_bowl, intro_balance').eq('id', userId).maybeSingle(),
