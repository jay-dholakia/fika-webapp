...
- **Score-only intake (private):** q13 country belief, q14 societal discussion style. These are used in the match score only. They are **not** included in reasons and **not** shown in the intro modal (treated as private).
...
| **q13_country_belief** | **Filter 7:** Do not match "Moving in the right direction" with "In need of major change." |

### 1.3 Backend: score weights (96% total; embed conditional)
...
| **q13_country_belief** | 2% | Same or adjacent on belief scale; Filter 7 blocks right direction vs major change. **Private — not in reasons.** (Question: "Right now, I believe the country is…" — Moving in the right direction, In need of major change, More stable than people think, Hard to define in one sentence, Prefer not to say.) |
| **q14_societal_discussion** | 2% | Compatibility (e.g. share my views vs opposing perspectives). **Private — not in reasons.** |

**Total:** 8 + 32 + 14 + 10 + 7 + 6 + 6 + 5 + 4 + 2 + 2 = 96% (embed is conditional).

**Note:** q13, q14 are **scoring only** — not included in reasons, not visible to the other user. Profession / university / major are in the score and remain visible in reasons/modal.
...
**Onboarding order (webapp):** Who you are (q2, q3, profession/school/major) → What you talk about (q5 topics, q13 country, q14 societal) → How you connect (q10, q4, q6) → Practicals (q9, q8) → Optional wrap (q11, q12, confirm).
...
q13, q14 are scoring only; do **not** include them in reasons or show in the intro modal.
...
- [x] **Backend:** Score weights: embed 8% conditional, q5 32%, q2 10%, q10 14%, q4 7%, q6 6%, distance 6%, q3_work_or_study 5%, q3_profession/university/major 4%, q13 2%, q14 2%. q13/q14 scoring only, not in reasons. q15 removed.
- [x] **Backend:** Implement full weight set (incl. q3 profession/school/major 4%, q13 2%, q14 2%). Do **not** include q13, q14 in reasons payload.
